...
export const getDashboardStats = asyncHandler(async (req, res) => {
  const [ordersRes, productsRes, usersRes, revenueRes, recentOrdersRes, statusRes] = await Promise.all([
...
export const getHeroConfig = asyncHandler(async (req, res) => {
  ...
  if (error || !data) {
    ...
    return res.json({
      headline:      'Andaaz',
      subheadline:   'اندازِ وقت',
      subtext:       'Premium watches crafted for those who understand that time is not just measured — it is worn.',
      image_url:     'https://images.unsplash.com/photo-1522312346375-d1a52e2b99b3?w=500&q=85',
      badge_text:    'New Arrival',
      badge_sub:     'Swiss Collection 2025',
      from_price:    '48000',
      discount_text: '',
      cta_text:      'Shop Now',
    });
