    ...
    <div className="min-h-screen bg-white">
      <div className="bg-black text-white py-12 md:py-16 px-5 md:px-8">
        <div className="max-w-4xl mx-auto">
          <p className="uppercase tracking-[0.35em] text-xs mb-2" style={{ color: "#d4af37" }}>Your Selection</p>
          <h1 className="text-4xl md:text-5xl font-black" style={{ fontFamily: "'Georgia', serif" }}>Cart</h1>
          <p className="text-gray-400 mt-1 text-sm">{cartItems.length} item{cartItems.length !== 1 ? "s" : ""}</p>
        </div>
      </div>
        ...
        {grouped.length === 0 ? (
          ...
          <div className="flex flex-col md:grid md:grid-cols-3 gap-8">
                ...
                {grouped.map((item) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="font-black mt-1 text-sm md:text-base" style={{ color: "#d4af37" }}>${item.price * item.qty}</p>
                    </div>
            ...
            <div className="md:col-span-1 order-first md:order-last">
              <div className="bg-gray-50 rounded-2xl p-5 md:p-6 md:sticky md:top-24">
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex justify-between text-gray-500">
                    <span>Shipping</span>
                    <span className={total >= 200 ? "text-green-600 font-medium" : ""}>
                      {total >= 200 ? "Free" : "$15"}
                    </span>
                  </div>
                  <div className="border-t pt-3 flex justify-between font-black text-base md:text-lg">
                    <span>Total</span>
                    <span style={{ color: "#d4af37" }}>${total >= 200 ? total : total + 15}</span>
                  </div>
                </div>
                {total < 200 && (
                  <p className="text-xs text-gray-400 mt-2 text-center">Add ${200 - total} more for free shipping</p>
                )}
                <motion.button whileTap={{ scale: 0.97 }}
                  className="w-full mt-5 py-4 rounded-xl font-bold text-sm uppercase tracking-widest text-black transition"
                  style={{ background: "#d4af37" }}>
                  Checkout
                </motion.button>
                <p className="text-xs text-gray-400 text-center mt-2">SSL encrypted · Secure checkout</p>
              </div>
            </div>
          </div>
        )}
