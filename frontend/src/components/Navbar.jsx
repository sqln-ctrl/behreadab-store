      ...
      >
        <div className="max-w-7xl mx-auto px-5 md:px-8 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4 text-lg text-gray-300">
            {/* Wishlist */}
            <motion.button whileHover={{ color: "#ef4444", scale: 1.15 }} whileTap={{ scale: 0.9 }}>
              <FaHeart />
            </motion.button>

            {/* Cart */}
            <Link to="/cart">
              <motion.button className="relative" whileHover={{ color: "#d4af37", scale: 1.15 }} whileTap={{ scale: 0.9 }}>
                <FaShoppingCart />
                <AnimatePresence>
                  {cartItems.length > 0 && (
                    <motion.span key={cartItems.length}
                      initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }}
                      className="absolute -top-2 -right-3 text-black text-xs w-5 h-5 rounded-full flex items-center justify-center font-black"
                      style={{ background: "#d4af37" }}>
                      {cartItems.length}
                    </motion.span>
                  )}
                      ...
                      <>
                        ...
                        {isAdmin && (
                          <Link to="/admin/dashboard">
                            <button className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition">
                              <FaShieldAlt className="text-yellow-500" /> Admin Panel
                            </button>
                          </Link>
                        )}
        ...
        {mobileOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed top-0 right-0 bottom-0 w-72 bg-black z-50 p-6 md:hidden"
            >
              <div className="flex justify-between items-center mb-8">
                <h2 className="text-white font-black text-xl" style={{ fontFamily: "'Georgia', serif" }}>
                  Watch<span style={{ color: "#d4af37" }}>Store</span>
                </h2>
              ...
              <ul className="space-y-1">
                ...
                {user ? (
                  <>
                    ...
                    {isAdmin && <li><Link to="/admin/dashboard" className="block px-4 py-3 font-medium rounded-xl hover:bg-white/5 transition" style={{ color: "#d4af37" }}>Admin Panel</Link></li>}
                    <li>
