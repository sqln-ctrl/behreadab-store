...
import {
  ...
  FaTimes, FaSignOutAlt, FaStore, FaChevronDown
} from "react-icons/fa";
...
const navGroups = [
  ...
  },
  {
    label: "Finance",
    items: [
      { to: "/admin/accounting", label: "Accounting", icon: <FaFileInvoiceDollar /> },
    ],
  },
];
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <nav className="flex-1 px-3 py-4 overflow-y-auto space-y-5">
        {navGroups.map((group) => (
          <div key={group.label}>
            <p className="text-xs font-semibold uppercase tracking-widest text-gray-600 px-4 mb-2">
              {group.label}
            </p>
      ...
      <div className="border-t border-white/10 p-4 flex-shrink-0">
        ...
        <div className="flex gap-2">
          <Link to="/" className="flex-1">
            <button className="w-full flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs text-gray-400 hover:bg-white/5 transition">
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <div className="flex-1 md:ml-60 flex flex-col min-h-screen">
        ...
        <header className="bg-white border-b px-5 md:px-8 py-4 flex items-center justify-between sticky top-0 z-20">
          ...
          <span className="text-xs bg-yellow-100 text-yellow-700 px-3 py-1 rounded-full font-bold uppercase tracking-wider">
            Admin
          </span>
        </header>
