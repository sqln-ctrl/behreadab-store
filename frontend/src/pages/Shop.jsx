...
const Shop = () => {
  ...
  const resetAll = () => {
    setSearch(""); setCategory("All"); setPriceRange([0, 500]); setSort("default"); setCurrentPage(1);
  };

  const SidebarContent = () => (
    <div className="space-y-8">
      <div>
        <h3 className="font-bold text-xs uppercase tracking-widest mb-3 text-gray-700">Search</h3>
        <SearchBar value={search} onChange={(v) => { setSearch(v); setCurrentPage(1); }} onClear={() => { setSearch(""); setCurrentPage(1); }} />
      </div>
      <div>
        <h3 className="font-bold text-xs uppercase tracking-widest mb-3 text-gray-700">Category</h3>
        <ul className="space-y-1">
          {categories.map((cat) => (
            <li key={cat}>
              <motion.button
                onClick={() => { setCategory(cat); setCurrentPage(1); setMobileFilterOpen(false); }}
                whileTap={{ scale: 0.97 }}
                className="w-full text-left px-4 py-2.5 rounded-xl text-sm font-medium transition-all"
                style={{ background: category === cat ? "#d4af37" : "transparent", color: category === cat ? "#000" : "#6b7280" }}
              >
                {cat}
      ...
      <div>
        <h3 className="font-bold text-xs uppercase tracking-widest mb-3 text-gray-700">Price</h3>
        <PriceFilter minPrice={0} maxPrice={500} value={priceRange} onChange={(r) => { setPriceRange(r); setCurrentPage(1); }} />
      </div>
      <motion.button
        onClick={resetAll}
        whileTap={{ scale: 0.97 }}
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div ref={headerRef} className="bg-black text-white py-12 md:py-16 px-5 md:px-8">
        ...
        >
          <p className="uppercase tracking-[0.35em] text-xs mb-2" style={{ color: "#d4af37" }}>Our Collection</p>
          <h1 className="text-4xl md:text-5xl font-black" style={{ fontFamily: "'Georgia', serif" }}>Shop Watches</h1>
          <p className="text-gray-400 mt-1 text-sm">{filtered.length} timepieces available</p>
        </motion.div>
      </div>
          ...
          <div className="md:col-span-3">
            ...
            <AnimatePresence mode="wait">
              {paginated.length === 0 ? (
                ...
                <motion.div
                  key={`${category}-${search}-${sort}-${currentPage}`}
                  initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
