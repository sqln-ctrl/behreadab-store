...
const Checkout = () => {
  ...
  const grouped = cartItems.reduce((acc, item) => {
    ...
    if (ex) ex.qty += 1; else acc.push({ ...item, qty: 1 });
    return acc;
  }, []);
  ...
  const validate = () => {
    ...
    if (!user && !guestEmail) e.guestEmail = "Email required for order updates";
    setErrors(e);
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm sticky top-24">
        ...
        <div className="border-t pt-3 space-y-2 text-sm">
          ...
          <div className="flex justify-between text-gray-500">
            <span>Shipping</span>
            <span className={shippingCost===0?"text-green-600 font-medium":""}>{shippingCost===0?"Free":`PKR ${Number(shippingCost).toLocaleString()}`}</span>
          </div>
          ...
          <div className="flex items-center gap-3">
            {["Delivery","Review","Done"].map((s, i) => (
              <div key={s} className="flex items-center gap-2">
                ...
                {i < 2 && <div className="w-5 h-px bg-white/20" />}
              </div>
            ))}
          </div>
        </div>
        ...
        <AnimatePresence mode="wait">
              ...
              <div className="md:col-span-2 space-y-5">
                ...
                {!user && (
                  <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5">
                    <p className="font-bold text-amber-800 text-sm mb-1">Guest Checkout</p>
                    <p className="text-amber-700 text-xs mb-3">You're ordering as a guest. Provide your email to receive order updates.</p>
                    <div>
                      <label className="text-xs font-semibold uppercase tracking-widest text-amber-700 mb-1.5 block">Email Address *</label>
                      <input type="email" value={guestEmail} onChange={e => { setGuestEmail(e.target.value); if (errors.guestEmail) setErrors(er => ({...er, guestEmail:""})); }}
                        placeholder="your@email.com"
                        className={`w-full border rounded-xl px-4 py-2.5 text-sm outline-none transition ${errors.guestEmail?"border-red-400":"focus:border-black border-amber-200"}`} />
                      {errors.guestEmail && <p className="text-red-500 text-xs mt-1">{errors.guestEmail}</p>}
                    </div>
                    <p className="text-xs text-amber-600 mt-3">
                      <a href="/register" className="underline font-medium">Create an account</a> to save your address and track orders easily.
                    </p>
                  </div>
                )}
                ...
                <div className="bg-white rounded-2xl p-6 shadow-sm">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {[
                      { label:"Full Name *",     key:"full_name",   col:"sm:col-span-2", placeholder:"Ali Khan" },
                      { label:"Phone *",          key:"phone",       col:"sm:col-span-2", placeholder:"+92-300-1234567" },
                      { label:"Street Address *", key:"street",      col:"sm:col-span-2", placeholder:"House 5, Street 10, DHA Phase 2" },
                      { label:"City *",           key:"city",        col:"",              placeholder:"Lahore" },
                      { label:"Province *",       key:"province",    col:"",              placeholder:"Punjab" },
                      { label:"Postal Code",      key:"postal_code", col:"sm:col-span-2", placeholder:"54000" },
                ...
                <div className="bg-white rounded-2xl p-6 shadow-sm">
                  ...
                  <div className="space-y-3">
                    <label className={`flex items-center gap-4 p-4 rounded-xl border-2 cursor-pointer transition ${payment==="cod"?"border-black bg-gray-50":"border-gray-200"}`}>
                      <input type="radio" name="pay" value="cod" checked={payment==="cod"} onChange={()=>setPayment("cod")} className="w-4 h-4 accent-black" />
                ...
                <div className="bg-white rounded-2xl p-5 shadow-sm">
                  ...
                  <textarea value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Special delivery instructions..." rows={2}
          ...
          {step === 1 && (
            <motion.div key="review" initial={{ opacity:0, x:20 }} animate={{ opacity:1, x:0 }} exit={{ opacity:0, x:-20 }}
              ...
              <div className="md:col-span-2 space-y-4">
                <div className="bg-white rounded-2xl p-5 shadow-sm">
                  <h2 className="font-black text-lg mb-3" style={{ fontFamily:"'Georgia', serif" }}>Review Your Order</h2>
                  <div className="space-y-1 text-sm">
                    ...
                    {!user && <p className="text-gray-500 mt-1">Email: <strong>{guestEmail}</strong></p>}
                    <p className="text-gray-500 mt-1">Payment: <strong>Cash on Delivery</strong></p>
                  </div>
                </div>
          ...
          {step === 2 && (
            <motion.div key="done" initial={{ opacity:0, scale:0.95 }} animate={{ opacity:1, scale:1 }}
              ...
              {orderId && <p className="text-xs text-gray-400 font-mono mb-2">Order #{orderId.slice(-8).toUpperCase()}</p>}
              {!user && guestEmail && <p className="text-xs text-gray-400 mb-6">Updates will be sent to {guestEmail}</p>}
              <div className="flex gap-3 justify-center mt-6">
                ...
                {user && <motion.button onClick={()=>navigate("/orders")} whileTap={{ scale:0.97 }} className="px-5 py-2.5 rounded-xl bg-black text-white text-sm font-bold">My Orders</motion.button>}
              </div>
            </motion.div>
          )}
